...
export function SmallCameraPreview({ videoRef, closeCamera, error }: SmallCameraPreviewProps) {
  ...
  return (
    <div className="relative mt-6 w-full aspect-video bg-physics-dark rounded-lg overflow-hidden flex items-center justify-center border-2 border-physics-navy animate-pulse-border shadow-lg">
      {error ? (
      ...
      {/* Overlay guide frame for the small preview with improved animation */}
      {isVideoVisible && (
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0 border-2 border-physics-gold/50 animate-glow rounded-lg"></div>
      ...
      <button 
        onClick={closeCamera}
        className="absolute top-2 right-2 p-2 bg-physics-dark/80 rounded-full z-10 shadow-md"
