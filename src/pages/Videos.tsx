  ...
  return <div className="min-h-screen bg-physics-navy flex flex-col relative">
      ...
      <main className="flex-1 p-6">
        <div className="max-w-4xl mx-auto">
          ...
          {selectedVideo && <div className="bg-physics-dark rounded-lg overflow-hidden mb-6">
              {selectedVideoIsYouTube ? <YouTubeEmbed videoUrl={selectedVideo} title={videos.find(v => v.url === selectedVideo)?.title || ""} /> : <VideoPlayerFixed src={selectedVideo} title={videos.find(v => v.url === selectedVideo)?.title || ""} />}
              <div className="p-4">
          ...
          {!selectedVideo && <div className="bg-physics-dark rounded-lg overflow-hidden">
              {filteredVideos.length === 0 ? <div className="p-8 text-center">
                  ...
                  {filteredVideos.map(video => <div key={video.id} className="p-4 hover:bg-physics-navy/30">
                      <div className="flex items-center">
                        <div className="mr-4 bg-physics-navy p-3 rounded-full cursor-pointer" onClick={() => handleSelectVideo(video)}>
                        ...
                        <div className="flex-1 cursor-pointer" onClick={() => handleSelectVideo(video)}>
                          <div className="flex items-center">
                            <h3 className="text-lg font-medium text-white">
                              {video.title}
                              {video.isYouTube && <span className="mr-2 text-xs bg-red-600 text-white px-2 py-0.5 rounded">
                                  YouTube
                                </span>}
                            </h3>
      ...
      {showAddForm && <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-physics-dark rounded-lg p-6 w-full max-w-md">
            ...
            {useYouTube ? <form onSubmit={handleAddVideo} className="space-y-4">
                <div>
